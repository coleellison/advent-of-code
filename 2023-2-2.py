input_text = """Game 1: 7 blue, 6 green, 3 red; 3 red, 5 green, 1 blue; 1 red, 5 green, 8 blue; 3 red, 1 green, 5 blue
...
def power(hands_str):
    ...
    for hand in hands:
        ...
        for i in range(len(draw_list)):
            if draw_list[i].isdigit():
                max_dict[draw_list[i + 1]] = max(max_dict[draw_list[i + 1]], int(draw_list[i]))
    power = 1
    for color in max_dict:
        power *= max_dict[color]
    return power

power_list = []
for i in range(100):
    power_list.append(power(hands_list[i]))
